...
def train_test_loading(path):
    x_train = np.load(path + 'X_train.npy')
    train_y_vector = np.load(path + 'y_train.npy').astype(int)
    x_test = np.load(path + 'X_test.npy')
    test_y_vector = np.load(path + 'y_test.npy').astype(int)
    class_column = 0
    labels = np.concatenate((train_y_vector, test_y_vector), axis=0)
    nclass = int(np.amax(labels)) + 1
    (num_instance, attr_num, attr_len) = x_train.shape
    train_x_matrix = x_train.reshape(num_instance, attr_num * attr_len)
    (num_instance, attr_num, attr_len) = x_test.shape
    test_x_matrix = x_test.reshape(num_instance, attr_num * attr_len)

    data_group = data_collection(train_x_matrix, np.squeeze(train_y_vector), class_column)
    data_group.test_x_matrix = test_x_matrix
    data_group.test_y_vector = np.squeeze(test_y_vector)
    # print(data_group.test_y_vector[0:10])
    # print(data_group.train_y_vector[0:10])
    # print(data_group.train_x_matrix[0:3, 0:5])
    # sdfsd
    return data_group, int(attr_num)

# convert 2d x matrix to 4d matrix
# method is "fcn": ins * attr_len * attr_num
# method is "cnn": ins * attr_len * attr_num * 1
# because fcn model combined all the attr_num at the first conv layer, it considers the attr_num as the channels
def data_group_processing(data_group, attr_num, method="fcn"):
    ...
    if train_x_matrix is not None:
        ...
        if "cnn" in method:
            train_x_matrix = train_x_matrix.reshape(train_row, attr_num, attr_len, 1)
            data_group.train_x_matrix = np.transpose(train_x_matrix, (0, 2, 1, 3))
        else:
            train_x_matrix = train_x_matrix.reshape(train_row, attr_num, attr_len)
            data_group.train_x_matrix = np.transpose(train_x_matrix, (0, 2, 1))
    ...
    if test_x_matrix is not None:
        ...
        if "cnn" in method:
            test_x_matrix = test_x_matrix.reshape(test_row, attr_num, attr_len, 1)
            data_group.test_x_matrix = np.transpose(test_x_matrix, (0, 2, 1, 3))
        else:
            test_x_matrix = test_x_matrix.reshape(test_row, attr_num, attr_len)
            data_group.test_x_matrix = np.transpose(test_x_matrix, (0, 2, 1))
    ...
    if valid_x_matrix is not None:
        ...
        if "cnn" in method:
            valid_x_matrix = valid_x_matrix.reshape(valid_row, attr_num, attr_len, 1)
            data_group.valid_x_matrix = np.transpose(valid_x_matrix, (0, 2, 1, 3))
        else:
            valid_x_matrix = valid_x_matrix.reshape(valid_row, attr_num, attr_len)
            data_group.valid_x_matrix = np.transpose(valid_x_matrix, (0, 2, 1))
