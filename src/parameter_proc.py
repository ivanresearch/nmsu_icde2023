from fileio.data_io import init_folder
from fileio.data_io import check_folder_exists
from fileio.data_io import check_file_exists
from data_setting import DataStru
from model_setting import ModelSetting


def read_parameters(parameter_file, function_keyword=""):
    if function_keyword == "":
        function_keyword = "all_feature_classification"
    data_stru = DataStru()
    ...
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#data_folder', '#attr_num', '#attr_len',
        '#num_classes', '#start_class', '#class_column', '#method',
        '#class_id', '#log_folder', '#obj_folder', '#cnn_setting_file',
        "#learning", "#dropout"
    ]
    ...
    for line in lines:
        ...
        if keyword == '#data_keyword':
            data_stru.data_key = line.strip()
        elif keyword == '#data_folder':
            data_stru.data_folder = line.strip()
        elif keyword == '#attr_num':
            data_stru.attr_num = int(line.strip())
        elif keyword == '#attr_len':
            data_stru.attr_len = int(line.strip())
        elif keyword == '#num_classes':
            data_stru.num_classes = int(line.strip())
        elif keyword == '#start_class':
            data_stru.min_class = int(line.strip())
        elif keyword == '#class_column':
            data_stru.class_column = int(line.strip())
        elif keyword == '#method':
            model_setting.model_key = line.strip()
            ...
            model_setting.learning_rate = float(line.strip())
        elif keyword == "#dropout":
            model_setting.dropout = float(line.strip())

    if data_stru.attr_num < 0 or data_stru.attr_len < 0 or (
            data_stru.num_classes < 0) or (data_stru.min_class < 0) or (
                data_stru.class_column < 0):
        raise Exception(
            "Wrong data paramters, please check the parameter file " +
            parameter_file)

    if model_setting.model_key == 'cnn' or model_setting.model_key == 'fcn':
        if check_file_exists(cnn_setting_file) is False:
            raise Exception("Missing cnn setting parameter file: " +
                            cnn_setting_file)

    log_folder = log_folder.replace('KEYWORD', data_stru.data_key)
    obj_folder = obj_folder.replace('KEYWORD', data_stru.data_key)
    ...
    model_setting.obj_folder = obj_folder
    model_setting.out_obj_folder = out_obj_folder
    model_setting.out_model_folder = out_model_folder
    model_setting.cnn_setting_file = cnn_setting_file
    return data_stru, model_setting
...
def read_genetic_cnn_parameter(parameter_file, function_keyword="genetic_cnn"):
    ...
    keyword_list = [
        '#data_sub_folder', '#data_keyword', '#data_folder', '#attr_num',
        '#attr_len', '#num_classes', '#start_class', '#class_column',
        '#method', '#log_folder', '#obj_folder', '#out_model_folder',
        '#cnn_setting_file'
    ]
...
def read_genetic_nn_parameter(parameter_file, function_keyword="genetic_nn"):
    ...
    keyword_list = [
        '#num_selected', '#data_keyword', '#data_folder', '#attr_num',
        '#attr_len', '#num_classes', '#start_class', '#class_column',
        '#log_folder', '#obj_folder', '#nn_setting_file'
    ]
    ...
    if check_file_exists(nn_setting_file) == False:
        raise Exception("Missing nn setting parameter file: " +
                        nn_setting_file)
...
# The function is used to read the input method key file
# The key file contains the data file we need to process
def read_nn_classification_parameter(parameter_file,
    ...
    keyword_list = [
        '#data_keyword', '#data_folder', '#attr_num', '#attr_len',
        '#num_classes', '#start_class', '#class_column', '#log_folder',
        '#obj_folder', '#nn_setting_file'
    ]
...
def read_project_feature_generation_parameter(
        parameter_file, function_keyword="projected_feature_generation"):
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#attr_num', '#attr_len', '#num_classes',
        '#start_class', '#class_column', '#method', '#pckl_folder',
        '#data_folder', '#log_folder', '#out_obj_folder', '#pckl_keyword'
    ]
    lines = open(parameter_file).readlines()
    data_keyword = ''
    keyword = ''
    ...
    if attr_num < 0 or attr_len < 0 or num_classes < 0 or start_class < 0 or class_column < 0 or check_folder_exists(
            pckl_folder) == False:
        raise Exception(
            "Wrong data paramters, please check the parameter file " +
            parameter_file)
...
def read_cnn_varying_classification_parameter(parameter_file):
    ...
    keyword_list = [
        '#data_keyword', '#data_folder', '#attr_num', '#attr_len',
        '#num_classes', '#start_class', '#class_column', '#method',
        '#log_folder', '#obj_folder', '#out_model_folder', '#cnn_setting_file'
    ]
...
def read_pure_feature_generation(parameter_file,
                                 function_keyword="pure_feature_generation"):
    lines = open(parameter_file).readlines()
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#data_folder', '#attr_num', '#attr_len',
        '#num_classes', '#start_class', '#class_column', '#method',
        '#log_folder', '#obj_folder'
    ]
...
# The key file contains the data file we need to process
# function keyword can be "projected_classification" or "global_classification"
def read_cnn_feature_generation(parameter_file,
                                function_keyword="cnn_feature_generation"):
    lines = open(parameter_file).readlines()
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#data_folder', '#num_classes', '#start_class',
        '#class_column', '#method', '#log_folder', '#obj_folder'
    ]
...
# The key file contains the data file we need to process
# function keyword can be "projected_classification" or "global_classification"
def read_feature_classification(parameter_file,
                                function_keyword="projected_classification"):
    lines = open(parameter_file).readlines()
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#data_folder', '#attr_num', '#attr_len',
        '#num_classes', '#start_class', '#class_column', '#top_k', '#method',
        '#class_id', '#log_folder', '#obj_folder', '#obj_keyword',
        '#cnn_setting_file'
    ]
    ...
    out_obj_folder = output_folder + "_top" + str(
        top_k) + "_cnn_obj_folder" + split_key
    out_model_folder = output_folder + "_top" + str(
        top_k) + "_cnn_model_folder" + split_key
...
# The function is used to read the input method key file
# The key file contains the data file we need to process
def read_grouped_projected_classification(
        parameter_file, function_keyword="grouped_projected_classification"):
    lines = open(parameter_file).readlines()
    split_key = '/'
    keyword_list = [
        '#data_keyword', '#data_folder', '#data_sub_folder', '#attr_num',
        '#attr_len', '#num_classes', '#start_class', '#class_column',
        '#method', '#log_folder', '#obj_folder', '#obj_sub_keyword',
        '#cnn_setting_file'
    ]
