...
class ModelSetting:
    def __init__(self, model_key="cnn", learning_rate=1e-3) -> None:
        self.model_key = model_key
        self.attn_key = 'none'
        self.batch_control = False
        self.learning_rate = learning_rate
        self.dropout = 0.5
        self.setting_file = "../parameters/cnn_model_parameter.txt"
        self.conv_kernel_list = None
        self.pool_rate_list = None
        self.feature_num_list = None

    def model_init(self):
        if self.model_key == "cnn" or self.model_key == "fcn":
    ...
    def read_setting(self):
        keyword_list = ['#kernel_list', '#pooling_list', '#feature_num_list', '#batch',
                        '#max_iter', '#stop_threshold', '#activation_func', '#std_value',
                                        '#same_size', '#feature_method', '#eval_method']
        first_delimiter = ', '
        second_delimiter = ' '
        keyword = ''
        conv_kernel_list = ''
        ...
        lines = open(self.setting_file).readlines()
        for line in lines:
            if line.startswith('#'):
                temp = line.strip()
            ...
            if keyword == '#kernel_list':
                self.conv_kernel_list = line.strip()
            elif keyword == '#pooling_list':
                self.pool_rate_list = line.strip()
            elif keyword == '#feature_num_list':
                self.feature_num_list = line.strip()
            elif keyword == '#batch':
                self.batch_size = int(line.strip())
            elif keyword == '#max_iter':
                self.max_iter = int(line.strip())
            elif keyword == '#stop_threshold':
                self.stop_threshold = float(line.strip())
            elif keyword == '#activation_func':
                self.activation = int(line.strip())
            elif keyword == '#std_value':
                self.std_value = float(line.strip())
            elif keyword == '#same_size':
                self.same_size = line.strip()
            elif keyword == '#feature_method':
                self.feature_method = line.strip()
            elif keyword == '#eval_method':
                self.eval_method = line.strip()

        if self.batch_size < 0 or self.max_iter < 0 or self.stop_threshold < 0 or self.activation < 0 or self.std_value < 0:
            raise Exception("Wrong data paramters, please check the parameter file " + self.setting_file)

        self.conv_kernel_list = self.conv_kernel_list.split(first_delimiter)
        self.pool_rate_list = self.pool_rate_list.split(first_delimiter)
        self.feature_num_list = self.feature_num_list.split(first_delimiter)
        ...
        self.conv_kernel_list = string_array_to_numpy_array(self.conv_kernel_list, second_delimiter, 2)
        self.pool_rate_list = string_array_to_numpy_array(self.pool_rate_list, second_delimiter, 2)
        self.feature_num_list = string_array_to_numpy_array(self.feature_num_list, second_delimiter, 1)

    def to_string(self):
        ret_str = "model key: " + str(self.model_key) + "\nattn_key: " + str(
            self.attn_key) + "\nbatch control: " + str(
                self.batch_control) + "\nlearning rate: " + str(
            self.learning_rate) + "\ndropout: " + str(
                self.dropout) + "\nlog_foler: " + str(
                    self.log_folder) + "\nobj_folder: " + str(self.obj_folder)
        if self.model_key == "cnn" or self.model_key == "fcn" or self.model_key == "fcn-mlstm":
            ret_str += "\nConv: " + str(self.conv_kernel_list)
            ret_str += "\nPool: " + str(self.pool_rate_list)
            ret_str += "\nFeature: " + str(self.feature_num_list)
        return ret_str
...
def string_array_to_numpy_array(input_str_array, delimiter=' ', col_num=2):
    ...
    for i in range(0, array_len):
        ...
        if len(element) != col_num:
            raise Exception("The column number should be " + str(col_num) +
                            " , please check your cnn setting parameter file")
        for item in element:
...
class cnn_parameters:
    ...
    def to_string(self):
        ...
        if self.activation_fun == 0:
            ...
            ret_str = ret_str + '\nactivation function: RELU'
        ret_str = ret_str + '\ninitial std value: ' + str(
            self.std_value) + '\ncnn same size or not: ' + str(
                self.same_size) + '\nfeature method: ' + str(
                    self.feature_method)
        ret_str = ret_str + '\neval method: ' + self.eval_method
...
def cnn_setting_clone(cnn_setting):
    return cnn_parameters(cnn_setting.conv_kernel_list,
                          cnn_setting.pool_rate_list,
                          cnn_setting.feature_num_list, cnn_setting.batch_size,
                          cnn_setting.max_iter, cnn_setting.stop_threshold,
                          cnn_setting.activation_fun, cnn_setting.std_value,
                          cnn_setting.same_size, cnn_setting.feature_method,
                          cnn_setting.eval_method, cnn_setting.out_obj_folder,
...
def return_cnn_setting_from_file(cnn_setting_file):
    keyword_list = [
        '#kernel_list', '#pooling_list', '#feature_num_list', '#batch',
        '#max_iter', '#stop_threshold', '#activation_func', '#std_value',
        '#same_size', '#feature_method', '#eval_method'
    ]
    first_delimiter = ', '
    ...
    if batch_size < 0 or max_iter < 0 or stop_threshold < 0 or activation_func < 0 or std_value < 0:
        raise Exception(
            "Wrong data paramters, please check the parameter file " +
            parameter_file)
    ...
    if len(conv_kernel_list) != len(pool_rate_list) or len(
            pool_rate_list) != len(feature_num_list):
        raise Exception(
            "Conv layers and pooling layers need to have the same number")

    conv_kernel_array = string_array_to_numpy_array(conv_kernel_list,
                                                    second_delimiter, 2)
    pool_rate_array = string_array_to_numpy_array(pool_rate_list,
                                                  second_delimiter, 2)
    feature_num_array = string_array_to_numpy_array(feature_num_list,
                                                    second_delimiter, 1)

    return cnn_parameters(conv_kernel_array, pool_rate_array,
                          feature_num_array, batch_size, max_iter,
                          stop_threshold, activation_func, std_value,
                          same_size, feature_method, eval_method)
...
def return_cnn_keyword(cnn_setting):
    ...
    for item in conv_kernel_list:
        cnn_keyword = cnn_keyword + '_c' + str(item[0]) + '-' + str(
            item[1]) + '_' + str(feature_num_list[feature_count])
...
def return_cnn_default_setting(conv_kernel_array=np.array([[1, 2], [1, 2]]),
                               pool_rate_array=np.array([[1, 2], [1, 2]]),
                               feature_num_array=np.array([2, 2]),
                               batch_size=100,
                               max_iter=200,
                               stop_threshold=0.9,
                               activation_func=0,
                               ...
                               feature_method='none',
                               eval_method='acc'):
    cnn_setting = cnn_parameters(conv_kernel_array, pool_rate_array,
                                 feature_num_array, batch_size, max_iter,
                                 stop_threshold, activation_func, std_value,
                                 same_size, feature_method, eval_method)
...
def return_nn_setting_from_file(nn_setting_file):
    keyword_list = [
        '#layer_list', '#batch_size', '#max_epoch', '#stop_threshold',
        '#activation_fun', '#std_value', '#eval_method', '#obj_folder'
    ]
    ...
    nn_setting = nn_parameters(layer_list, batch_size, max_epoch,
                               stop_threshold, activation_fun, std_value,
                               eval_method)
